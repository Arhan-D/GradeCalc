...
function viewStudentList() {
  ...
  if (students.length) {
    ...
    students.forEach((oAnkiOgrenci, index) => {
      ...
      //Öğrenci bilgisini içeren eleman oluşturma studen-item classı yani
...
let sepet = [{
  id:1,
  name:'adidas Vs Pace Erkek Spor',
  adet:3
},
{
  id:2,
  name:'Nike Air Force Günlük Spor',
  adet:2
},
]

const arttirmaIslemi = sepet.map((item,index)=>{
  
  if (index === 1) {
    
    console.log(`${item.name} ürününden sepetimde ${item.adet} tane var}`)
    item.adet = (item.adet + 2) 
  }
})

console.log('sepetimdeki urunler', sepet) */
